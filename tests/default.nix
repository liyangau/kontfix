...
let
  ...
  testConfigurations =
    let
      caseFiles = builtins.filter (file: lib.hasSuffix ".nix" file) (
        builtins.attrNames (builtins.readDir ./cases)
      );
    in
    map (file: lib.removeSuffix ".nix" file) caseFiles;
    ...
    let
      ...
      errorCaseFiles =
        if builtins.pathExists errorCasePath then
          ...
          [ ];
    in
    map (file: lib.removeSuffix ".nix" file) errorCaseFiles;
  ...
  createTestConfiguration =
    ...
    kontfixLib.kontfixConfiguration {
      inherit system;
      modules = [
    ...
    in
    {
      ...
      program = toString (
        pkgs.writers.writeBash "test-all-builds" ''
          ...
          for config in ${lib.concatStringsSep " " testConfigurations}; do
    ...
    { pkgs, system }:
    {
      ...
      program = toString (
        pkgs.writers.writeBash "test-all-errors" ''
          ...
          if [[ -z "${lib.concatStringsSep " " errorTestConfigurations}" ]]; then
          ...
          TEST_FAILED=0
          for config in ${lib.concatStringsSep " " errorTestConfigurations}; do
            echo ""
  ...
  generateBuildApps =
    { pkgs, system }:
    let
      individualApps = lib.listToAttrs (
        map (configName: {
...
in
{
  apps = generateBuildApps { inherit pkgs system; };
